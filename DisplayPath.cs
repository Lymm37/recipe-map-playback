...
namespace Lymm37.PotionCraft.RecipeMapPlayback
{
    class DisplayPath
    {
        ...
        static Dictionary<string, string> ingredientTranslation = new Dictionary<string, string> { { "Leaf", "Terraria" }, { "Waterbloom", "Waterbloom" }, { "GoblinMushroom", "Goblin Shroom" }, { "Firebell", "Firebell" }, {"DryadMushroom", "Dryad's Saddle" }, { "Windbloom", "Windbloom"}, { "CliffFungus", "Brown Mushroom"}, { "Marshrooms", "Marshroom"}, {"Thornstick", "Thornstick" }, { "Wierdshroom", "Weirdshroom"}, {"Tangleweeds","Tangleweed" }, { "Thistle", "Thunder Thistle"}, { "LumpyBeet", "Lumpy Beet"}, {"IceDragonfruit", "Ice Fruit" }, {"GreenMushroom", "Green Mushroom" }, {"RedMushroom", "Red Mushroom" }, { "GreyChanterelle", "Shadow Chanterelle"}, { "GraveTruffle", "Grave Truffle"}, {"SulphurShelf", "Sulphur Shelf" }, {"LavaRoot", "Lava Root" }, {"Refruit", "Hairy Banana" }, {"Goldthorn", "Goldthorn" }, {"WitchMushroom", "Witch Mushroom" }, { "BloodyRoot", "Bloodthorn"}, {"EarthCrystal", "Earth Pyrite" }, {"FrostSapphire", "Frost Sapphire" }, { "FireCitrine", "Fire Citrine"}, {"Crystal", "Cloud Crystal" }, {"BloodRuby", "Blood Ruby" } };
                    ...
                    if (asCSV)
                    {
                        ...
                        {  
                            ...
                            for (int i = 0; i < points.Count; i++)
                            {
                                ...
                                writer.WriteLine(i + "," + p.name + "," + p.x + "," + p.y + "," + p.typeCode + "," + p.grindState + "," + p.value + "," + p.health + "," + p.rotation + "," + p.teleportStatus + "," + p.whirlpoolStatus + "," + p.philSaltStatus + "," + p.cumulativeDistance);
                            }
                            writer.Close();
                        }
                        catch (Exception ex)
